...
import { createDisclosure, getCases, type CaseItem } from "@/lib/api"
...
export function CreateModel({ onBack, onSubmit }: CreateModelProps) {
  ...
  const [sourceCase, setSourceCase] = useState<CaseItem | null>(null)
  const [submitting, setSubmitting] = useState(false)

  useEffect(() => {
    getCases({ m05_status: "completed", pageSize: 1 })
      .then((data) => {
        const first = data.list[0] || null
        setSourceCase(first)
        if (first?.patent_type?.includes("实用")) setPatentType("utility")
      })
      .catch(console.error)
  }, [])

  const sourceInfo = {
    sourceId: sourceCase?.case_id || "暂无可建模来源",
    caseId: sourceCase?.case_id || "-",
    clientName: sourceCase?.client_name || "-",
    contactName: sourceCase?.contact_person || "-",
    title: sourceCase?.case_name || "-",
    modelType: sourceCase?.source_type === "presale" ? "售前咨询" : "已立案",
    submittedAt: sourceCase?.create_time || "-",
  }

  const materials = useMemo(() => [
    { id: "1", name: `${sourceInfo.caseId}-技术交底信息`, type: "document", size: sourceCase?.tech_field || "待补充", status: sourceCase ? "valid" : "warning" },
    { id: "2", name: `${sourceInfo.caseId}-客户与联系人信息`, type: "document", size: sourceCase?.contact_person || "待补充", status: sourceCase?.contact_person ? "valid" : "warning" },
    { id: "3", name: `${sourceInfo.caseId}-业务来源信息`, type: "document", size: sourceCase?.source_type || "系统记录", status: "valid" },
  ], [sourceCase, sourceInfo.caseId])
  ...
  const handleCreateModel = async () => {
    ...
    try {
      await createDisclosure({
        ...
        source_content: [
          `技术主题：${sourceCase.case_name}`,
          `技术领域：${sourceCase.tech_field || ""}`,
          `专利类型：${sourceCase.patent_type || ""}`,
          `客户：${sourceCase.client_name || ""}`,
          `建模模式：${modelMode}`,
        ].join("\n"),
      })
      onSubmit()
    } catch (err) {
      console.error("创建交底模型失败：", err)
    } finally {
      setSubmitting(false)
    }
  }
              ...
              <div className="grid grid-cols-2 gap-6">
                <div className="space-y-4">
                  <div>
                    ...
                    <Input value={sourceInfo.sourceId} disabled className="mt-1 bg-[#F9FAFB]" />
                  </div>
                  <div>
                    <Label className="text-[#6B7280]">案件编号</Label>
                    <Input value={sourceInfo.caseId || "-"} disabled className="mt-1 bg-[#F9FAFB]" />
                  </div>
                  <div>
                    <Label className="text-[#6B7280]">客户名称</Label>
                    <Input value={sourceInfo.clientName} disabled className="mt-1 bg-[#F9FAFB]" />
                  </div>
                </div>
                <div className="space-y-4">
                  <div>
                    <Label className="text-[#6B7280]">联系人</Label>
                    <Input value={sourceInfo.contactName} disabled className="mt-1 bg-[#F9FAFB]" />
                  </div>
                  <div>
                    <Label className="text-[#6B7280]">技术主题</Label>
                    <Input value={sourceInfo.title} disabled className="mt-1 bg-[#F9FAFB]" />
                  </div>
                  <div>
                    ...
                    <div className="mt-1">
                      <span
                        className={`text-sm px-3 py-1 rounded ${
                          sourceInfo.modelType === "已立案"
                            ? "bg-[#DCFCE7] text-[#166534]"
                            : "bg-[#DBEAFE] text-[#1E40AF]"
                        }`}
                      >
                        {sourceInfo.modelType}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex items-start gap-2">
                  ...
                  <div>
                    ...
                    <div className="text-xs text-[#A16207] mt-1">
                      系统会基于后端案件字段生成交底模型，缺失字段会在后续补全环节处理
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div className="p-4 bg-[#F9FAFB] rounded-lg">
                    ...
                    <div className="text-sm font-medium text-[#111827] mt-1">{sourceInfo.sourceId}</div>
                  </div>
                  <div className="p-4 bg-[#F9FAFB] rounded-lg">
                    <div className="text-xs text-[#6B7280]">案件编号</div>
                    <div className="text-sm font-medium text-[#111827] mt-1">{sourceInfo.caseId}</div>
                  </div>
                  <div className="p-4 bg-[#F9FAFB] rounded-lg">
                  ...
                  <div className="p-4 bg-[#F9FAFB] rounded-lg">
                    ...
                    <div className="text-sm font-medium text-[#111827] mt-1">{materials.length} 份</div>
                  </div>
                </div>
      ...
      <div className="flex justify-between">
        ...
        <div className="flex gap-3">
          ...
          {currentStep < 5 ? (
            ...
            <Button className="bg-[#10B981] hover:bg-[#059669]" onClick={handleCreateModel} disabled={!sourceCase || submitting}>
              {submitting ? "创建中..." : "创建模型"}
            </Button>
          )}
        </div>
