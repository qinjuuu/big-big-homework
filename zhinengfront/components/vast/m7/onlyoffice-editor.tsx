...
export function OnlyOfficeEditor({
  ...
  useEffect(() => {
    if (initialContent !== undefined) {
      setContent(initialContent || EMPTY_CONTENT)
      return
    }
    if (writingId === undefined || writingId === null || writingId === "") {
    ...
    ;(async () => {
      try {
        const detail = await getWritingById(writingId)
        if (cancelled) return
        const next =
          (documentType === "claims" ? detail?.claim_content : detail?.spec_content) ||
          detail?.spec_content ||
          ""
        setContent(next && next.trim() ? next : EMPTY_CONTENT)
      } catch (err) {
  ...
  }, [writingId, initialContent, documentType])
  ...
  const handleAiAssist = async () => {
    ...
    try {
      if (writingId !== undefined && writingId !== null && writingId !== "") {
        const res = await fetch(`/api/ai/suggestion?writing_id=${encodeURIComponent(String(writingId))}`)
        if (res.ok) {
          const json = await res.json().catch(() => null)
          const suggestion =
            (json && (json.data?.suggestion || json.data?.advice || json.suggestion || json.advice)) || ""
          setAiSuggestion(typeof suggestion === "string" ? suggestion : "")
        } else {
          setAiSuggestion("")
        }
      } else {
