...
const SECTION_CONFIG: Array<{ id: string; title: string; field: keyof DisclosureItem | string }> = [
  { id: 'section-1', title: '技术问题', field: 'tech_problem' },
  { id: 'section-2', title: '技术背景', field: 'background_tech' },
  { id: 'section-3', title: '现有技术缺点', field: 'defects' },
  { id: 'section-4', title: '发明目的', field: 'purpose' },
  { id: 'section-5', title: '技术方案', field: 'solution' },
  { id: 'section-6', title: '关键保护点', field: 'keypoints' },
  { id: 'section-7', title: '有益效果', field: 'effect' },
  { id: 'section-8', title: '实际产品', field: 'embodiments' },
]

export function DisclosureReviewPage({ onNavigate, disclosureId }: DisclosureReviewPageProps) {
  ...
  useEffect(() => {
    ...
    getDisclosureById(disclosureId).then(setDisclosure).catch(console.error)
  }, [disclosureId])

  const disclosureSections = SECTION_CONFIG.map((cfg) => {
    const content = (disclosure as any)?.[cfg.field] || ''
    return { id: cfg.id, title: cfg.title, complete: Boolean(content), content }
  })

  const reviewChecklist = SECTION_CONFIG.map((cfg) => ({
    item: `是否有${cfg.title}`,
    status: Boolean((disclosure as any)?.[cfg.field]),
  })).concat([{ item: '是否有图纸材料', status: Boolean((disclosure as any)?.drawings) }])
