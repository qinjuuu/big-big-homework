...
export function PresaleDashboard({ onNavigate }: PresaleDashboardProps) {
  ...
  useEffect(() => {
    getCases({ pageSize: 100 }).then((data) => setTickets(data.list)).catch(console.error)
  }, [])

  const countByStatus = (status: string) => tickets.filter((item) => item.m05_status === status).length

  const stats = [
    { label: "今日新建", value: tickets.length, icon: Plus, color: "bg-[#2F80ED]", change: `+${tickets.length}`, page: "presale-list" },
    { label: "待分配工程师", value: countByStatus("assigning"), icon: Users, color: "bg-[#F59E0B]", change: `+${countByStatus("assigning")}`, page: "presale-pending" },
    { label: "待检索", value: countByStatus("searching"), icon: FileText, color: "bg-[#8B5CF6]", change: `+${countByStatus("searching")}`, page: "presale-processing" },
    { label: "待确认", value: countByStatus("confirming"), icon: Clock, color: "bg-[#06B6D4]", change: `${countByStatus("confirming")}`, page: "presale-confirm" },
    { label: "待立案", value: countByStatus("filing"), icon: Send, color: "bg-[#10B981]", change: `+${countByStatus("filing")}`, page: "presale-to-filing" },
    { label: "超期未处理", value: countByStatus("rejected"), icon: AlertCircle, color: "bg-[#EF4444]", change: `${countByStatus("rejected")}`, page: "presale-pending" },
  ]
  ...
  const getStatusBadge = (status: string) => {
    const configs: Record<string, { label: string; className: string }> = {
      ...
      rejected:      { label: "不立案", className: "bg-[#F5F5F5] text-[#8C8C8C] border-[#D9D9D9]" },
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        <span className="font-medium text-sm text-[#111827]">{ticket.case_id}</span>
                        {getStatusBadge(ticket.m05_status)}
                      </div>
                      <div className="text-xs text-[#6B7280]">{ticket.client_name || "-"} · {ticket.patent_type}</div>
                    </div>
                  </div>
                  <div className="text-right space-y-0.5">
                    <div className="text-xs text-[#6B7280]">{ticket.update_time || ticket.create_time}</div>
                    <div className="text-xs text-[#9CA3AF]">
                      销售：<span className="text-[#CA8A04] font-medium">{ticket.sales_person || "-"}</span>
                      <span className="mx-1">·</span>
                      客服：<span className="text-[#0369A1] font-medium">{ticket.service_rep || "-"}</span>
                    </div>
                  </div>
                </div>
              ))}
            ...
            <div className="pt-4 border-t">
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between text-sm cursor-pointer hover:text-[#2F80ED]" onClick={() => onNavigate("presale-pending")}>
                  ...
                  <Badge variant="outline" className="bg-[#FEF3C7] text-[#D97706]">{countByStatus("assigning")}</Badge>
                </div>
                <div className="flex items-center justify-between text-sm cursor-pointer hover:text-[#2F80ED]" onClick={() => onNavigate("presale-confirm")}>
                  ...
                  <Badge variant="outline" className="bg-[#DBEAFE] text-[#2563EB]">{countByStatus("confirming")}</Badge>
                </div>
                <div className="flex items-center justify-between text-sm cursor-pointer hover:text-[#2F80ED]" onClick={() => onNavigate("presale-to-filing")}>
                  ...
                  <Badge variant="outline" className="bg-[#D1FAE5] text-[#059669]">{countByStatus("filing")}</Badge>
                </div>
              </div>
            </div>
          </CardContent>
