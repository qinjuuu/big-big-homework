...
export function WaitingFilingList({ onNavigate, onViewDetail }: WaitingFilingListProps) {
  ...
  const calculateDaysWaiting = (time?: string) => {
    if (!time) return 0
  ...
  const toWaitingItem = (item: CaseItem): WaitingFilingItem => {
    const daysWaiting = calculateDaysWaiting(item.update_time || item.create_time)
    const missingInfo = [
      !item.tech_field ? "技术领域" : "",
      !item.engineer ? "专利工程师" : "",
      !item.contact_person ? "联系人信息" : "",
    ].filter(Boolean)
    ...
    return {
      ...
      daysWaiting,
      isOverdue: daysWaiting > 3,
      manager: item.sales_person || item.service_rep || "-",
      engineer: item.engineer || "-",
      signedDate: item.create_time || "-",
      missingInfo,
  ...
  const fetchWaitingCases = async () => {
    setLoading(true)
    try {
      const data = await getCases({ m05_status: "filing", keyword: searchText || undefined, pageSize: 100 })
      setCases(data.list)
    } catch (err) {
      console.error("获取待立案列表失败：", err)
    } finally {
      setLoading(false)
    }
  }
  ...
  const data = useMemo(() => cases.map(toWaitingItem), [cases])

  // 统计数据
  const stats = {
    all: data.length,
    today: data.filter(item => item.daysWaiting === 0).length,
    overdue: data.filter(item => item.isOverdue).length,
    mine: data.filter(item => item.manager !== "-").length,
    needInfo: data.filter(item => item.missingInfo.length > 0).length,
  }
  ...
  const filteredData = data.filter((item) => {
    ...
    if (activeTab === "mine") return item.manager !== "-"
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <Button variant="outline" className="gap-2">
            ...
            <span onClick={fetchWaitingCases}>{loading ? "刷新中" : "刷新"}</span>
          </Button>
          <Button variant="outline" className="gap-2">
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <DialogFooter>
            ...
            <Button className="bg-[#52C41A] hover:bg-[#52C41A]/90" onClick={() => {
              if (currentItem) {
                updateCaseStatus(currentItem.id, "待交底", "completed", "前端用户")
                  .then(fetchWaitingCases)
                  .catch((err) => console.error("立案失败：", err))
              }
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <DialogFooter>
            ...
            <Button variant="destructive" onClick={() => {
              if (currentItem) {
                updateCaseStatus(currentItem.id, "已归档", "rejected", "前端用户")
                  .then(fetchWaitingCases)
                  .catch((err) => console.error("不立案失败：", err))
              }
              setShowNotFilingDialog(false)
              onNavigate("m09-scrap-cases")
            }}>
